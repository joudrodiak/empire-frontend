...
export function RowActions({ onView, onEdit, onDelete, deleteLabel = 'this item', size = 15 }: {
  ...
  const btn = 'rounded-md p-1.5 text-empire-text-muted transition-colors hover:bg-empire-elevated/70'
  ...
  return (
    <div className="flex items-center gap-0.5" onClick={e => e.stopPropagation()}>
      ...
      {onDelete && (
        <button onClick={() => setConfirming(true)} title="Delete" className={`${btn} hover:text-empire-red-bright`}>
      ...
      <Modal open={confirming} onClose={() => !busy && setConfirming(false)} title="Confirm delete" icon={<EmpireIcon name="alert" size={18} />} width="max-w-sm">
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button onClick={() => setConfirming(false)} disabled={busy} className="rounded px-3 py-2 text-xs uppercase tracking-widest text-empire-text-muted hover:text-empire-text disabled:opacity-50">Cancel</button>
          <button
            onClick={async () => { setBusy(true); try { await onDelete?.() } finally { setBusy(false); setConfirming(false) } }}
            disabled={busy}
            className="rounded bg-empire-red/80 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-empire-text transition-colors hover:bg-empire-red disabled:opacity-50"
          >
